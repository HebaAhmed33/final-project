...
export default function ConfigurationPage() {
  ...
  // ── EXISTING Upload state (UNCHANGED) ──────────────────────────────────
  ...
  // ── EXISTING Config submit (UNCHANGED) ─────────────────────────────────
  ...
  const modeCardStyle = (active) => ({
    flex: 1, padding: "1.25rem", borderRadius: "0.75rem", cursor: "pointer",
  ...
  return (
    <PageContainer>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "1.5rem" }}>
        <button type="button" onClick={() => { setMode("upload"); setScanError(null); }} style={modeCardStyle(mode === "upload")}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
            <svg style={{ width: "22px", height: "22px", color: mode === "upload" ? "var(--primary)" : "var(--text-muted)" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
            <span style={{ fontSize: "1.05rem", fontWeight: 700, color: mode === "upload" ? "var(--primary)" : "var(--text-main)" }}>Upload Configuration Files</span>
          </div>
          <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", margin: 0 }}>Upload JSON, YAML, ENV, or raw config files for analysis</p>
        </button>

        <button type="button" onClick={() => { setMode("live"); setError(null); }} style={modeCardStyle(mode === "live")}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
            <svg style={{ width: "22px", height: "22px", color: mode === "live" ? "var(--primary)" : "var(--text-muted)" }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
            <span style={{ fontSize: "1.05rem", fontWeight: 700, color: mode === "live" ? "var(--primary)" : "var(--text-main)" }}>Live Configuration Scan</span>
          </div>
          <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", margin: 0 }}>SSH into a server and scan its live configuration remotely</p>
        </button>
      </div>

      {/* ──────────── ERROR (shared) ──────────── */}
      {(mode === "upload" ? error : scanError) && (
        <div style={{ padding: "1rem", marginBottom: "1.5rem", background: "rgba(239,68,68,0.05)", border: "1px solid #EF4444", color: "#EF4444", borderRadius: "0.5rem", display: "flex", alignItems: "center", gap: "0.75rem" }}>
          ...
          <span style={{ fontSize: "0.95rem" }}>{mode === "upload" ? error : scanError}</span>
        </div>
      )}

      {/* ══════════════════════════════════════════════════════════════════
          OPTION 1: EXISTING Upload Flow (COMPLETELY UNCHANGED LOGIC)
          ══════════════════════════════════════════════════════════════════ */}
      {mode === "upload" && (
            ...
            <div className="card" style={{ padding: "1.5rem" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "0.75rem" }}>
                {[
                  ...
                  <button key={fw.key} type="button" onClick={() => setConfigFramework(fw.key)} style={fwBtnStyle(configFramework === fw.key)}>
      ...
      {/* ══════════════════════════════════════════════════════════════════
          OPTION 2: NEW Live Configuration Scan (ISOLATED)
          ══════════════════════════════════════════════════════════════════ */}
      {mode === "live" && (
